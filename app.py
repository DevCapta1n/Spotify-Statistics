...
def display_stats(user_id):
    ...
    if request.method == 'POST':
        ...
        artists = TopArtist.query.filter_by(user_id=user_id, time_range=artist_range).order_by(top_artists.c.rank.asc()).all()
        tracks = TopTrack.query.filter_by(user_id=user_id, time_range=track_range).order_by(top_tracks.c.rank.asc()).all()
